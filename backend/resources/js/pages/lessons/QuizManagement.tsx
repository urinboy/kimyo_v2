...
const QuizManagement = () => {
  ...
  useEffect(() => {
    if (quizData?.data?.quiz?.questions && !isInitialized) {
      ...
      const mapped: Question[] = quizData.data.quiz.questions.map(q => ({
        ...
        points: q.points,
        order: q.order,
        translations: q.translations as unknown as Record<number, any>,
        options: q.options.map(o => ({
          id: o.id,
          is_correct: o.is_correct,
          translations: o.translations as unknown as Record<number, any>
        }))
      }));
      setQuestions(mapped);
  ...
  const createQuizMutation = useMutation({
    mutationFn: (data: { lesson_id: number }) => quizApi.create(data),
  ...
  const handleCreateQuiz = () => {
    createQuizMutation.mutate({ lesson_id: lessonId });
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      {!quiz ? (
        <GlassCard className="py-20 text-center flex flex-col items-center">
          <AlertCircle className="w-16 h-16 text-amber-500/50 mb-6" />
          <h2 className="text-2xl font-bold mb-2 text-app-primary">{t('quiz.no_quiz_title')}</h2>
          <p className="text-app-muted max-w-md mb-8">{t('quiz.no_quiz_desc')}</p>
          <button 
            onClick={handleCreateQuiz} 
            disabled={createQuizMutation.isPending}
            className="btn-primary flex items-center gap-2"
          >
            {createQuizMutation.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : <Plus className="w-5 h-5" />}
            {t('quiz.init_quiz')}
          </button>
        </GlassCard>
      ) : (
